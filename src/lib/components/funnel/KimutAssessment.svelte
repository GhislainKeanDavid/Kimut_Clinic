<script>
	...
	async function fetchMonthAvailability(monthStr) {
		...
		isLoadingMonth = true;
		try {
			const pt = formData.preferred_pt;
			const url =
				pt && pt !== 'any'
					? `/api/calendar-availability?month=${monthStr}&pt=${pt}`
					: `/api/calendar-availability?month=${monthStr}`;
	...
	async function fetchDateSlots(dateStr) {
		...
		try {
			...
			const url =
				pt && pt !== 'any'
					? `/api/calendar-availability?date=${dateStr}&pt=${pt}`
	...
	const allFullyBookedDates = $derived(Object.values(monthFullyBooked).flat());
		...
		try {
			...
			if (result.success) {
				...
				if (result.reason === 'duplicate') {
					submitError =
						'It looks like you already have a booking with this email and time. Email us at info@kimutclinic.com if this is a mistake.';
				} else {
	...
	<div class="mx-auto max-w-4xl px-6">
		<div class="mb-16 text-center">
			...
			<div class="mt-6 flex flex-wrap items-center justify-center gap-3">
				<span
					class="rounded-full border border-Background/20 bg-Background/10 px-4 py-1.5 font-mono text-xs text-Background/80"
					>~ 3 minutes</span
				>
				<span
					class="rounded-full border border-Background/20 bg-Background/10 px-4 py-1.5 font-mono text-xs text-Background/80"
					>PRC-Licensed Therapists</span
				>
				<span
					class="rounded-full border border-Background/20 bg-Background/10 px-4 py-1.5 font-mono text-xs text-Background/80"
					>Response within 24 hrs</span
				>
			</div>
		</div>
		...
		{#if !isSuccess}
			...
			>
				...
				{#if isSubmitting}
					<div
						class="absolute inset-0 z-50 flex flex-col items-center justify-center gap-4 rounded-[2rem] bg-white/95 backdrop-blur-sm"
					>
						<div
							class="h-14 w-14 animate-spin rounded-full border-4 border-Mist/40 border-t-Accent"
						></div>
							...
							<div class="mx-auto max-w-xl space-y-12">
								<div>
									<label for="duration" class="mb-4 block font-medium"
										>How long have you had this issue?</label
									>
									<input
										type="range"
										id="duration"
									...
									/>
									<div class="mt-2 text-center text-lg text-Accent font-mono">
										{formData.durationDays} days
									</div>
								</div>
								<div>
									<label for="severity" class="mb-4 block font-medium"
										>How severe is the pain?</label
									>
									<input
										type="range"
										id="severity"
						...
						<div class="step">
							...
							<div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
								<div class="space-y-1">
									...
									<input
										...
										class="w-full rounded-xl border border-Mist/60 bg-transparent px-4 py-3 outline-none focus:border-Accent focus:ring-1 focus:ring-Accent"
									/>
								</div>
								<div class="grid grid-cols-2 gap-4">
									<div class="space-y-1">
										<label class="text-sm text-Dark/70" for="age"
											>Age <span class="text-Accent">*</span></label
										>
										<input
											id="age"
											type="number"
									...
									<div class="space-y-1">
										<label class="text-sm text-Dark/70" for="sex"
											>Sex <span class="text-Accent">*</span></label
										>
										<select
											id="sex"
								...
								<div class="space-y-1">
									<label class="text-sm text-Dark/70" for="email"
								...
								<div class="space-y-1">
									<label class="text-sm text-Dark/70" for="phone"
							...
							<div class="mt-6 flex justify-end gap-4">
								<button class="px-6 py-2 text-Dark/60 hover:text-Dark" onclick={prevStep}
									>Back</button
								>
								<MagneticButton class="px-8 py-3" onclick={nextStep} disabled={!step3Valid}
						...
						<div class="step">
							...
							<div class="mb-8 space-y-2">
								...
								<textarea
									...
									rows="3"
									class="w-full rounded-xl border border-Mist/60 px-4 py-3 placeholder-Dark/30"
									placeholder="Optional notes..."
								></textarea>
							</div>

							<p class="mb-8 text-xs text-Dark/50">
								By submitting, you agree to our Privacy Policy. Information is encrypted and compliant
								with RA 10173 (Data Privacy Act of 2012).
							</p>

							<div class="flex justify-between gap-4">
								<button
									class="px-6 py-2 text-Dark/60 hover:text-Dark"
									onclick={prevStep}
		...
		{:else}
			<!-- SUCCESS STATE -->
			<div
				class="mx-auto max-w-lg rounded-[2rem] border border-Mist/60 bg-white p-12 text-center shadow-sm"
			>
				...
				<h3 class="mb-4 font-serif text-3xl italic text-Dark">
					Thank you, {formData.full_name.split(' ')[0]}.
				</h3>
				<p class="mb-8 text-Dark/70">
					Your assessment has been received. Our PRC-licensed therapist will review your details and
				...
				<button
					class="font-medium text-Accent transition-colors hover:text-Primary inline-flex items-center gap-2"
				>
